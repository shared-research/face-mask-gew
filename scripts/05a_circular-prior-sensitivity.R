...
fit_ri_tas_maskint <- readRDS(file.path("models", "theta", "fit_ri_tas_maskint.rds"))
fit_ri_aq_maskint <- readRDS(file.path("models", "theta", "fit_ri_aq_maskint.rds"))
...
# Model 2b - tas * mask * intensity -------------------------------------------------

prior_von_mises_tas_maskint_un <- c(
  # theta
  prior(normal(0, 5), class = "b", dpar = "", coef = "Intercept"),
  prior(normal(0, 5), class = "b", dpar = "", coef = "mask1"),
  prior(normal(0, 5), class = "b", dpar = "", coef = "intensity1"),
  prior(normal(0, 1), class = "b", dpar = "", coef = "tas"),
  prior(normal(0, 5), class = "b", dpar = "", coef = "mask1:intensity1"),
  prior(normal(0, 1), class = "b", dpar = "", coef = "mask1:intensity1:tas"),
  prior(normal(0, 1), class = "b", dpar = "", coef = "mask1:tas"),
  prior(normal(0, 1), class = "b", dpar = "", coef = "intensity1:tas"),
  
  # kappa
  prior(normal(0, 5), class = "b", dpar = "kappa", coef = "Intercept"),
  prior(normal(0, 5), class = "b", dpar = "kappa", coef = "mask1"),
  prior(normal(0, 5), class = "b", dpar = "kappa", coef = "intensity1"),
  prior(normal(0, 1), class = "b", dpar = "kappa", coef = "tas"),
  prior(normal(0, 5), class = "b", dpar = "kappa", coef = "mask1:intensity1"),
  prior(normal(0, 1), class = "b", dpar = "kappa", coef = "mask1:intensity1:tas"),
  prior(normal(0, 1), class = "b", dpar = "kappa", coef = "mask1:tas"),
  prior(normal(0, 1), class = "b", dpar = "kappa", coef = "intensity1:tas")
)

fit_ri_tas_maskint_un <- brm(fit_ri_tas_maskint$formula,
                     data = dat_fit,
                     family = von_mises(link = "tan_half", link_kappa = "log"),
                     chains = chains,
                     prior = prior_von_mises_tas_maskint_un,
                     cores = cores,
                     iter = iter,
                     backend = "cmdstanr",
                     threads = threading(6),
                     sample_prior = samp_prior,
                     file = "models/theta/fit_ri_tas_maskint_un",
                     save_pars = save_pars(all = TRUE),
                     seed = seed)

success_step(fit_ri_tas_maskint_un)
...
# Model 3b - aq * mask * intensity ---------------------------------------------------

prior_von_mises_aq_maskint_un <- c(
  # theta
  prior(normal(0, 5), class = "b", dpar = "", coef = "Intercept"),
  prior(normal(0, 5), class = "b", dpar = "", coef = "mask1"),
  prior(normal(0, 5), class = "b", dpar = "", coef = "intensity1"),
  prior(normal(0, 1), class = "b", dpar = "", coef = "aq"),
  prior(normal(0, 5), class = "b", dpar = "", coef = "mask1:intensity1"),
  prior(normal(0, 1), class = "b", dpar = "", coef = "mask1:intensity1:aq"),
  prior(normal(0, 1), class = "b", dpar = "", coef = "mask1:aq"),
  prior(normal(0, 1), class = "b", dpar = "", coef = "intensity1:aq"),
  ...
  prior(normal(0, 1), class = "b", dpar = "kappa", coef = "aq"),
  prior(normal(0, 5), class = "b", dpar = "kappa", coef = "mask1:intensity1"),
  prior(normal(0, 1), class = "b", dpar = "kappa", coef = "mask1:intensity1:aq"),
  prior(normal(0, 1), class = "b", dpar = "kappa", coef = "mask1:aq"),
  prior(normal(0, 1), class = "b", dpar = "kappa", coef = "intensity1:aq")
)

fit_ri_aq_maskint_un  <- brm(fit_ri_aq_maskint$formula,
                             data = dat_fit,
                             family = von_mises(link = "tan_half", link_kappa = "log"),
                             chains = chains,
                             prior = prior_von_mises_aq_maskint_un,
                             cores = cores,
                             iter = iter,
                             backend = "cmdstanr",
                             threads = threading(6),
                             sample_prior = samp_prior,
                             file = "models/theta/fit_ri_aq_maskint_un",
                             save_pars = save_pars(all = TRUE),
                             seed = seed)

success_step(fit_ri_aq_maskint_un)
